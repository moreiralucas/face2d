...
def matchWithCrossValidation(X,Y):
    labels = np.ones(Y.shape) * -1
    indexs = filter(lambda indexs: indexs.shape[0] > 1, [np.where(Y == label)[0] for label in np.unique(Y)])
    cycle = itertools.cycle(range(0, min([i.shape[0] for i in indexs])))
    for x in indexs:
...
if __name__=='__main__':
    ...
    print 'Stats by Face'
    accuracy = statsByNFace(X,Y)
    print 'CM'
    preds,reals = matchWithCrossValidation(X,Y)
    plt.figure()
    cm_avg = np.zeros((50,50))
    for i in range(preds.shape[0]):
        cm = confusion_matrix(reals[i],preds[i])
        cm_avg = np.add(cm,cm_avg)
    for yi in range(cm_avg.shape[0]):
        for xi in range(cm_avg.shape[1]):
            if (xi % 4 == 0):
                print ''
            print (yi+1),(xi+1),(cm_avg[yi,xi])
